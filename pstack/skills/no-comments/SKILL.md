...
1. Use the custom `comment-sicko` agent when the runtime loads it. Otherwise spawn a general read-only worker and tell it to read the `comment-sicko` skill. Pass the scope. Do not restate its rules.
