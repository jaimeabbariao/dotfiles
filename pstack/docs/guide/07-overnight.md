...
keep going until done using the runtime's native goal or wake mechanism. if you're truly stuck after a few hours, stop and write up why.
```
...
- The [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md) uses the host's native goal, automation, routine, or loop mechanism to re-check the finish condition. If the host has none, it keeps the same predicate in the task and checks it before every iteration.
...
Before the skill hands back its summary, it spawns an isolated reviewer to read the trail and the active task transcript. It prefers another confirmed model family when the host supports one and otherwise discloses the single-family pass. The reply ends with an Attention section listing what deserves your scrutiny. Read that section first, then the log rows it points at. You're auditing decisions, not re-reading the whole night.
