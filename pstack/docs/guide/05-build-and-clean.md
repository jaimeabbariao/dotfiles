...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) cleans the diff before each commit and applies [`/unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. Cursor users may add `/deslop` from `cursor-team-kit`; on Claude or Codex, ask for the same outcome in plain words: remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
