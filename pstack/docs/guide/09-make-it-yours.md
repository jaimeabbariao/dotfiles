...
You don't describe your style, because [`/automate-me`](../../skills/automate-me/SKILL.md) reads the task history that the active runtime makes available for this workspace. It looks for repeated preferences in replies, delegation, verification, code, prose, and process, then asks you which patterns are really you. It drafts `<your-name>-mode/SKILL.md` in the active runtime's project skill directory, runs the draft through [`/unslop`](../../skills/unslop/SKILL.md), and opens a PR from a worktree so you review it like any other change. If scoped history is unavailable, it asks for examples instead of scanning global transcripts.
