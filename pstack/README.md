...
**pstack is my answer.** these are the same skills i use everyday to ship high quality code at Cursor. this adaptation packages them for Claude, Codex, and Cursor. the goal is not to maximize loc, in fact it's the opposite. pstack helps you write less, but higher quality code.
...
**use the models your runtime actually provides.** pstack inherits the parent model by default. on a host that can spawn both Claude and Codex models, judgment panels can use both families. single-family hosts run the same workflow with independent passes and say when cross-family review was unavailable.

fork it. improve it. make it yours. PRs are welcome! 

## install

The repository contains three manifests over one shared `skills/` tree:
...
For Codex, add `pstack` to a local plugin marketplace and install it from that marketplace. Cursor users can continue to install the upstream package with `/add-plugin pstack`.

## get started

two steps:

1. run [`setup-pstack`](./skills/setup-pstack/SKILL.md) and choose `inherit`, `claude`, `codex`, or `mixed` when the host supports it.
2. use [`poteto-mode`](./skills/poteto-mode/SKILL.md) whenever you're doing anything that requires rigor.

Invocation syntax depends on the host:

| Host | Setup | Main mode |
|---|---|---|
| Claude Code | `/pstack:setup-pstack` | `/pstack:poteto-mode` |
| Codex | `$pstack:setup-pstack` | `$pstack:poteto-mode` |
| Cursor | `/setup-pstack` | `/poteto-mode` |

new here? the [pstack guide](./docs/guide/README.md) walks you through a first real task, from setup and prompting through verification and overnight runs.

that's it. the other skills are situational; the mode skill uses them for you as needed. out of the box every role inherits the parent model. [`setup-pstack`](./skills/setup-pstack/SKILL.md) writes a project-local `.pstack/models.md` with exact identifiers confirmed by the active runtime.
...
[`poteto-mode`](./skills/poteto-mode/SKILL.md) can use the host's native goal, automation, routine, or loop mechanism for long-running work. those features are accelerators, not requirements.
...
| [`/make-bot-ui`](./skills/make-bot-ui/SKILL.md) | you want a page or dashboard whose buttons wake a Cursor automation over a webhook, including the sender-key handoff and Tailscale. |
...
pstack also ships a custom [`poteto-agent`](./agents/poteto-agent.md) for hosts that load plugin agents. Codex parents can instead spawn a general worker and instruct it to read the portable [`poteto-agent` skill](./skills/poteto-agent/SKILL.md). both paths load `poteto-mode` before work.

[`poteto-mode`](./skills/poteto-mode/SKILL.md), the custom agent, and the portable worker skill route through the same wrapper.

pstack also ships [Comment Sicko](./agents/comment-sicko.md) plus a portable [`comment-sicko` skill](./skills/comment-sicko/SKILL.md). usually invoke it through [`no-comments`](./skills/no-comments/SKILL.md), not directly.
...
- Cursor users can add `deslop`, `control-cli`, and `control-ui` from `cursor-team-kit`.
- Claude and Codex users should use equivalent native browser, terminal, review, and skill-creator capabilities when available.
- Core pstack workflows do not require those optional integrations.

install `cursor-team-kit` alongside pstack only when running in Cursor and you want that full set.

## why are there no planning skills?

Claude, Codex, and Cursor all provide ways to plan work. personally, i don't believe in planning by default. the best spec is code. if you do want to make a plan, [`poteto-mode`](./skills/poteto-mode/SKILL.md) covers it.
...
models are configurable too. run [`setup-pstack`](./skills/setup-pstack/SKILL.md). it detects exact identifiers exposed by the active runtime and writes `.pstack/models.md`. every skill inherits the parent when the file or a role is absent.

## automations

pstack also preserves the upstream, Cursor-specific [benny automation pack](./automations/benny/). benny is not part of the portable Claude/Codex core and its files are not registered as skills.
