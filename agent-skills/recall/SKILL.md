...
Use only conversation history that the active runtime explicitly exposes for this workspace or account. Prefer a task-list/read API over raw transcript files. When the runtime provides an active transcript path, use that path only. Never scan home-directory history or unrelated projects. If no history source is available, use the current conversation plus live repository state and name that limitation.
...
3. Fan out across the available history. Use the runtime's native delegation tool when the corpus is large, with each worker taking a time slice. Search the topic first, read only matching conversations and relevant regions, and skip the current task plus obvious worker, eval, and test noise. Each returns the same schema, one block per task: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the runtime's task or conversation identifier. For one or two tasks, search directly. Keep raw history out of the main context.
