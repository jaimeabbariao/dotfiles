...
4. Verify each component against its baseline via image diff on the matching surface via the control capability. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. Iterate per component until the diff is zero.
