...
5. **Spawn one blinded judge** per the **arena** skill's Phase C. Prefer another confirmed model family when the host supports it; otherwise use an isolated pass and disclose the single-family limitation. The judge sees outputs by sanitized label and the rubric, never a model name.
6. **Verify the chain from the run record, not self-report.** Read each candidate's task record or transcript only when the runtime explicitly exposes it for the active workspace. Never scan unrelated project history. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
