...
This skill orchestrates three parts: an inline mining pass, the runtime's skill-creator workflow when available, and the **unslop** skill. It sequences them; it doesn't replace them.
...
Use only conversation history the runtime explicitly exposes for the active workspace or account. Prefer a task-list/read API. If no history source exists, mine the current conversation and state the narrower evidence base. Never scan unrelated project history.
...
Mining misses intent that hasn't come up yet. Use the runtime's structured-question capability when available; otherwise ask concise questions in chat.
...
The **jimmy** skill shows the shape. Read it for granularity. Don't copy its content; the user's rules are not the same as jimmy's.

### 4. Draft the skill

Use the runtime's skill-creator workflow to author the skill. Placement:

- Path: preserve an existing mode skill's category. For a new mode, use the active runtime's project skill directory unless the user asks for a personal skill.
...
- Invocation policy: keep automatic discovery unless the user explicitly requests an explicit-only mode and the runtime supports that policy.

### 5. Iterate on prose

Apply the **unslop** skill and the skill-creator's writing guidelines to every line. Both apply to any agent-read prose, not just skills.
...
- The **jimmy** skill: example of the output shape.
- The **unslop** skill: prose discipline for every line.
- The active runtime's skill-creator workflow: skill authoring process and writing guidelines.
