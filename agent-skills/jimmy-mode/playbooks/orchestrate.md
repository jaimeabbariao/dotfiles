...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the worker's status in the runtime task dashboard. Transcript mtime is not liveness.
