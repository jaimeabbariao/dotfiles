...
name: make-bot-ui
description: >-
  Cursor-only workflow for building a custom UI that wakes a Cursor automation
  over a webhook. Do not use in Claude or Codex runtimes.
---
# How to make a bot UI

This skill depends on Cursor automation APIs and secret-request cards. If the active runtime is Claude or Codex, stop and explain that this optional upstream integration is Cursor-only. Do not emulate secret handling in chat.
...
Store `{url, key}` in that UI's own directory. Buttons POST to this local server. The local server, not the browser, POSTs to the Cursor automation webhook.
