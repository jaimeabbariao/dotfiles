---
name: jimmy
description: Jimmy's engineering workflow for concise communication, deliberate delegation, simple code, and verified work. Use for $jimmy or requests to work in Jimmy's style.
---
...
Resolve paths in this skill and its playbooks relative to the `jimmy` skill directory unless a path is explicitly rooted at `<repo>`.

Every named skill in this package resolves to `../<skill-name>/SKILL.md`. Read that bundled sibling by path. Do not substitute a same-named skill from another plugin.
...
- Any prose surface → the **unslop** skill. Your reply is a prose surface; write it per **Writing the reply**. Agent-facing prose also follows the runtime's skill-authoring guidance when available.
- Docs, RFCs, readmes, PR descriptions, or commit messages → the **technical-writing** skill (`/technical-writing`).
- Before commit → use `deslop` when the runtime provides it; otherwise inspect the diff for narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
...
Resolve the version-control workflow in this order:

1. Follow the user's explicit choice.
2. Follow repository instructions or established repository tooling.
3. Use Graphite when `gt` is available and the repository is tracked by Graphite.
4. Otherwise use Git and the available forge capability.
...
**Always pause** when an action needs new authority or is irreversible, including force-pushes to shared branches, deploys, data deletion, and messages to other people.
