...
1. Locate the prior trail. Use a transcript or task-read capability only when the runtime explicitly exposes it for the active workspace, or use a remote-task URL or pushed branch. Never scan unrelated project history. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long record in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
