...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Use the runtime's native spawn, wait, resume, and result tools. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out, to conserve context. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer: each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (nest only when the runtime supports it, to a maximum depth of 3). Rolls up aggregates at wave boundaries; never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window; never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Use local or remote execution according to the unit's repository, tools, credentials, and UI needs. Briefs inline what remote workers cannot read and point at repository paths for shared grounding. Prefer fewer, broader workers and one writer per worktree or branch. Prefer another confirmed model family for independent verification when available; otherwise disclose the single-family pass.
...
2. **Install the runtime.** Run `orch init` when the helper is available. Otherwise create the store files described above directly. Open the trail via the show-me-your-work skill, write the standing orders before any spawn, and seed `frontier.json` from existing PRs. Use `orch frontier set --repo <repo-dir>` when available.
...
- On a completion notification, record the same fields as `orch inbox push <agent> <unit> <status> [--report PATH]`, through the helper when available or as one inbox pointer file. Return to what you were doing. Never deep-review inline; a completion that needs review becomes a verifier unit. Never review a diff inside a drain.
...
- Each drain classifies every pointer (landed, needs-verify, failed, zombie, noise), updates `units.tsv` and `ledger.tsv` through `orch` when available or through direct single-writer edits, regenerates `status.md`, then spawns the next wave in one message.
...
- The frontier is a computed object, never narrative. Recompute `frontier.json` through the selected version-control workflow after every merge and stack mutation: ordered PR list, branch names, head SHAs, a generation number, and the lowest unmerged PR. Resolve it where that workflow has authoritative stack metadata. Error rather than guess when the checkout cannot resolve the submitted PRs.
- Exactly one stacker per stack may mutate its topology; record the holder and selected workflow in the standing orders. Run expensive restacks away from the coordinator's interactive environment when the runtime supports that placement.
- Workers never rebase or mutate stack topology. Babysitters follow `playbooks/babysit.md`, one per stack, scoped to one immutable frontier generation; they report conflicts to the stacker rather than restacking.
...
Scale verification to the unit. When VERIFY is a single cheap command, the worker runs it and reports the output, and the coordinator spot-checks receipts; a dedicated verifier agent (preferably on another confirmed model family) is for units whose verification is expensive, judgment-laden, or high-blast-radius. A verifier agent whose entire product would be rerunning one command is ceremony, not verification.

Write ledger rows with `orch ledger record` when available or update `ledger.tsv` directly. Check the current PR and head SHA with `orch ledger check` when available or read the keyed row. `ledger.tsv` has one row per verdict, keyed by PR number plus head SHA: `live-ui-verified | unit-test-verified | type-check-only | verifier-blocked | verifier-failed`. CI green is an input to a verdict, not a verdict. Behavioral work needs better than `type-check-only`. `verifier-blocked` is not a pass; respawn when the environment heals. `verifier-failed` gets a fix unit, not a re-verify. A worker may self-report; a verifier overrides it on the same key. A new head SHA voids the row, so re-verify after restack. The ledger answers "was this verified", not memory and not the transcript.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only through the ledger, `units.tsv`, the resolved forge, pushed branches, and the worker's status in the runtime task dashboard. Transcript mtime is not liveness.
