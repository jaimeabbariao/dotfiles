...
1. Snapshot and audit. Record `df -h /`, then run `scripts/worktree-audit.sh` (principle-build-the-lever). It reads paths from `git worktree list`, never hand-typed, since a hand-typed `myrepo-worktrees/x` misses one in a runtime-managed worktree directory (principle-encode-lessons-in-structure). It classifies each worktree by size, age, merge state, uncommitted work, and PR state, then suggests a bucket. Include recent task ownership only when the active runtime exposes repository-scoped history; otherwise omit it.
