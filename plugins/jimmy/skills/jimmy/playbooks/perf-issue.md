...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation when available using the configured perf-issue model (default `inherit-parent`); otherwise implement directly. Review the diff and capture a post-fix trace.
