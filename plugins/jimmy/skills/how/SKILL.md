...
Spawn all explorers concurrently with the runtime's native delegation tool. Keep them read-only. Use the configured how-explorer model when its exact identifier is confirmed; otherwise inherit the parent model.
...
Spawn a single read-only worker that explores and explains in one pass. Use the configured how-explainer model when its exact identifier is confirmed; otherwise inherit the parent model.
...
Once all explorers return, spawn a single read-only worker to synthesize their findings into one coherent explanation. Use the configured how-explainer model when its exact identifier is confirmed; otherwise inherit the parent model.
