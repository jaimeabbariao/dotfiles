...
Read `../jimmy/references/runtime-compatibility.md` before selecting models or spawning workers.
...
Pull PR bodies and discussion through the available forge capability for substantive commits. When authenticated `gh` is available, use:
...
Before spawning investigators, inspect the active runtime's available tools, connectors, plugins, and MCP resources. Use an official tool-discovery capability when one is exposed. Do not assume another runtime's filesystem layout.
...
Launch all matching investigators concurrently with the runtime's native delegation tool. One investigator per category lets each specialize in one tool's query vocabulary and result shape. Don't ask one worker to cover multiple connectors. Use the configured why-investigators model when its exact identifier is confirmed; otherwise inherit the parent model. Give investigators access to the read tools their source requires, but instruct them not to mutate external systems or local files.
...
1. **Source control investigator**. Git history, the available forge capability for PRs, code comments, and tests. Always run this pass when a repository is present. It is best at surfacing *implementation-time rationale captured during review*. PR descriptions state the problem, review threads debate alternatives, inline comments encode non-obvious constraints, test names encode motivating edge cases, and commit messages link tickets or incidents. This evidence is trustworthy because it ties directly to the diff that shipped.
...
Spawn one synthesizer worker. Use the configured why-synthesizer model when its exact identifier is confirmed; otherwise inherit the parent model. Give it the read access needed to spot-verify citations and instruct it not to write.
