...
At the end of the run, before handing back, check the log told the truth. Use the active task transcript only when the runtime explicitly exposes it. Never scan global history or unrelated projects. When no transcript is exposed, audit against the current conversation, tool results, diffs, and artifacts. Walk the log against what actually happened:
...
Before handing back, spawn an independent reviewer. Prefer a different confirmed model family from the one that did the work when the host supports it. On a single-family host, use an isolated pass and disclose that cross-family review was unavailable. The reviewer reads the audit trail and the available run record, then flags what the user should pay attention to. This is not a redo of the work, but a scan for what's suboptimal or risky.
