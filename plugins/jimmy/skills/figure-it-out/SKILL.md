...
Open a todolist whose first item is to read the Principles section of the **jimmy** skill. Then add the phases below as todos.
...
- For one-way-door design decisions, run the **architect** skill (it runs **arena**) with diverse, isolated candidates and a read-only judge. Prefer another confirmed model family for the judge when the host supports it; otherwise disclose the single-family pass. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
